...
export const buildDrillBar = (
  ...
  return {
    ...
    yAxis: {
      type: "value",
      name: (aggregationMethod + " of " + measureCol)
        .replace(/_/g, " ")
        .toUpperCase(),
      nameLocation: "middle",
      nameGap: 60,
...
export const buildDrillMultiline = (
  timeseries,
  activeMetric,
  drillTitle,
  aggMethod,
  ...
  const processedSeries = (rawSeries || []).map((s) => {
    ...
    if (aggMethod === "sum") data = s.sum;
      ...
      data = s.sum.map((sum, i) =>
        s.count[i] > 0 ? round(sum / s.count[i], 2) : 0,
      );
...
export const buildDrillHeatmap = (
  heatmapData,
  xCol,
  yCol,
  measureCol,
  ...
  return {
    ...
    visualMap: {
      ...
      bottom: 10,
      inRange: {
        color: ["#FCEBEB", "#F09595", "#E24B4A", "#A32D2D", "#501313"],
      },
      textStyle: { color: "#374151" },
    },
    series: [
...
export const buildDrillBubble = (
  ...
  if (!bubbleData || !Array.isArray(bubbleData) || bubbleData.length === 0)
    return {};
  ...
  const series = bubbleData.map((d, i) => ({
    ...
    symbolSize: (data) =>
      Math.max(12, Math.min(80, (data[2] / maxSize) * 70 + 10)),
    itemStyle: {
...
export const buildDrillScatter = (
  rawData,
  xCol,
  yCol,
  colorCol,
...
export const buildDrillSunburst = (
  sunburstData,
  measureCol,
  drillTitle,
  drillPath,
  ...
  const updateNodes = (nodes) => {
    return nodes.map((node) => {
      ...
      let val = baseSum;
      if (aggMethod === "avg" || aggMethod === "mean")
